...
pub fn lt_proc(args: &[Value]) -> Result<Value, String> {
    ...
    return Ok(Value::Boolean(
        vs.iter().zip(vs.iter().skip(1)).all(|(v1, v2)| v1 < v2),
    ));
}
...
pub fn car_proc(args: &[Value]) -> Result<Value, String> {
    ...
    match &args[0] {
        &Value::List(ref vs) if vs.is_empty() => Err("Cannot take car of empty list".to_owned()),
...
pub fn cdr_proc(args: &[Value]) -> Result<Value, String> {
    ...
    match &args[0] {
        &Value::List(ref vs) if vs.is_empty() => Err("Cannot take cdr of empty list".to_owned()),
...
mod tests {
    ...
    #[test]
    fn test_cdr() {
        let cases = vec![("(cdr '((a) b c d))", "(b c d)"), ("(cdr '(1 . 2))", "2")];
